...
他の TOML 設定ファイルを読み込んでマージします (オプショナル)。

- 値は、読み込む設定ファイルの **glob パターン** の配列です (例: `["./shared.toml", "./conf.d/*.toml"]`)。
- パターンは、この `include` を記述したファイルからの **相対パス** で指定します。
- `include` で指定されたファイルが先に読み込まれ、現在のファイルの内容で **上書き** されます (後勝ちマージ)。配列も上書きされます。
- 循環参照は検出され、警告と共にスキップされます。

#### 設定例

```toml
# このファイルの場所からの相対パスで指定
include = ["./shared/common.toml", "./configs/*.toml"]
